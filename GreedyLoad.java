...
public class GreedyLoad {
	...
	public GreedyLoad(int pNum, float balance, int vNum, String edgePath, String data, int ipercent, String inputOrder)
			throws IOException {
		...
		if (ipercent != 100) {
			File file = new File(
					"D:\\dataset\\greedy\\" + data + "2-" + inputOrder + "-partition-" + pNum + "-" + ipercent + "%");
			if (!file.exists()) {
				file.mkdir();
			}
		} else {
			File file = new File("D:\\dataset\\greedy\\" + data + "2-" + inputOrder + "-partition-" + pNum);
			if (!file.exists()) {
				file.mkdir();
			}
		}
		for (int i = 0; i < pNum; i++) {
			if (ipercent != 100) {
				...
				out[i] = new BufferedWriter(
						new OutputStreamWriter(new FileOutputStream("D:\\dataset\\greedy\\" + data + "2-" + inputOrder
								+ "-partition-" + pNum + "-" + ipercent + "%" + "\\partition" + i + ".txt")));
			} else {

				out[i] = new BufferedWriter(new OutputStreamWriter(new FileOutputStream("D:\\dataset\\greedy\\" + data
	...
	public static void main(String[] args) throws IOException {
		...
		int ipercent = 100;// 初始分区为百分之几50,80,100
		int[] ps = { /*2, 4, 8, 16,*/ 32};
		for (String data : datas) {
			...
			for (int pNum : ps) {
				String edgePath;
				if (ipercent != 100) {
					edgePath = "D:\\dataset\\greedy\\" + data + "2-" + inputOrder + "-" + ipercent + "%.txt";
				} else {
					edgePath = "D:\\dataset\\greedy\\" + data + "2-" + inputOrder + ".txt";
				}
				GreedyLoad greedy = new GreedyLoad(pNum, balance, vNum, edgePath, data, ipercent, inputOrder);
